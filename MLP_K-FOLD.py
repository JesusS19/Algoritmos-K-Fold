# -*- coding: utf-8 -*-
"""
Created on Fri May 23 15:48:33 2025
...
# Import the MLPClassifier algorithm
...
# --- Flatten sequences for the MLP algorithm ---
# Traditional ML algorithms like MLP expect a 2D input (samples, flattened_features).
...
# 5. Define the MLPClassifier model
# These parameters are based on your MLP.py file.
# 'random_state' ensures reproducibility.
mlp_model = MLPClassifier(hidden_layer_sizes=(16, 8), activation='relu', solver='adam', max_iter=500, random_state=42)
...
print(f"\n======== Running K-Fold Cross-Validation for: MLPClassifier ========")
...
fold = 1
# Iterate through the folds using the flattened data for MLP
for train_index, test_index in kf.split(X_seq_flattened, y_seq_original):
    ...
    # Create a new instance of the MLP model for each fold
    # This ensures that the training of each fold is independent.
    model_instance = MLPClassifier(hidden_layer_sizes=(16, 8), activation='relu', solver='adam', max_iter=500, random_state=42)
...
print(f"\n--- Overall Results for MLPClassifier ---")
...
plt.title(f"Overall Confusion Matrix - MLPClassifier (K-Fold Cross-Validation)")
plt.show()

print("\nMLPClassifier model evaluation completed.")